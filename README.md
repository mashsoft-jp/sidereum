...
- 土星の情報パネルの「環を探る」から、環の局所的な氷粒子を間近で眺める試作表示へ切り替えられる。輪へ近づいて暗転し、粒子の間へ入る演出付き。戻る際は元の視点まで引く（動きを減らす設定では演出を省略）。ドラッグで見回し、移動の停止・画像保存・元の視点への復帰に対応。粒子は不規則な立体で、メートル単位で厚さ20mに収め、環の中央から眺める。スクリーンセーバーのランダムな景色にも含まれる。土星は中心距離120,000kmからの角直径で描く。粒子の形・密度・照明は模式表現（個体形状の実測復元や衝突計算ではない）。組成の参考: [NASA Cassini — Rings](https://science.nasa.gov/mission/cassini/science/rings/)
