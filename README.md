...
高解像度ON時、マウス操作に対応したPCでGPUの最大テクスチャサイズが8192以上なら、地球の地表だけ `tex/8k/earth.jpg` (8192×4096) を使用します。スマートフォン・タブレット (デスクトップUAのiPadを含む) は4K、通常設定は全端末2Kです。雲・夜景・他天体は高解像度でも4Kです。8Kの取得・GPUへの転送に失敗した場合、そのセッションは地球も4Kに戻します。地表1枚の8K化でGPUメモリはミップ込み約128MiB増加します。新しいURLを追加したため既存画像のキャッシュは維持します。
