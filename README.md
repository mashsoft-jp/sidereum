...
- 土星の情報パネルの「環を探る」から、環の局所的な氷粒子を間近で眺める試作表示へ切り替えられる。輪へ近づいて暗転し、粒子の間へ入る演出付き。戻る際は暗転中に元の視点へ復元し、通常画面をフェードインする（動きを減らす設定では演出を省略）。ドラッグで見回し、移動の停止・画像保存・元の視点への復帰に対応。粒子は不規則な立体で、メートル単位で厚さ20mに収め、環の中央から眺める。スクリーンセーバーのランダムな景色にも含まれる。土星は中心距離120,000kmからの角直径で描く。粒子の形・密度・照明は模式表現（個体形状の実測復元や衝突計算ではない）。組成の参考: [NASA Cassini — Rings](https://science.nasa.gov/mission/cassini/science/rings/)
...
写真ビューには M1・M13・M16（創造の柱）・M31・M42・M45・M51・M57 の8点と、日本語・英語の短い解説を収録。写真と解説の出典は各ビュー内で表示します。
...
- メニューの**天文カレンダー**で、その年に起きることを一覧表示(上の年をタップして切替、「見る」でその場面へ移動)
...
高解像度ON時、マウス操作に対応したPCでGPUの最大テクスチャサイズが8192以上なら、地球・月・火星・水星の地表を接近時だけ8K (8192×4096) に切り替えます。画面内の半径120px以上で350ms留まると取得し、80px未満に離れると4Kへ戻します。対象が複数ある場合は最も大きく見える1天体だけ使用します。8Kの取得・画像展開・128行ずつの切り出しはWeb Worker内で行い、切り出したImageBitmapの所有権を画面側へ移して別フレームでGPUへ転送します。Workerが使えない場合は4Kを維持します。完成までは4Kを表示します。4Kを控えとして保持し、離れた天体の8Kはフェード終了後にGPUから解放します。

スマートフォン・タブレット (デスクトップUAのiPadを含む) は4K、通常設定は全端末2Kです。雲・夜景・法線図・他天体は高解像度でも4Kです。8Kの取得・GPUへの転送に失敗した場合、その天体はセッション中4Kを使います。保持する8K画像1枚はミップ込みで最大約171MiBのGPUメモリを使用します。

月はNASA CGI Moon Kitの2025年版で2K・4K・8Kを統一しています。元画像・利用条件・加工内容は [素材の記録](ASSET_RIGHTS.md) と [マニフェスト](licenses/asset-manifest.json) を参照してください。
