...
高解像度ON時、マウス操作に対応したPCでGPUの最大テクスチャサイズが8192以上なら、地球・月・火星・水星の地表を接近時だけ8K (8192×4096) に切り替えます。画面内の半径120px以上で350ms留まると取得し、80px未満に離れると4Kへ戻します。対象が複数ある場合は最も大きく見える1天体だけ使用します。8Kは非同期に画像を展開し、128行ずつ別フレームでGPUへ転送します。完成までは4Kを表示します。4Kを控えとして保持し、離れた天体の8Kはフェード終了後にGPUから解放します。
