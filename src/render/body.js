  ...
  const LBL_SEL   = 0;   // 選択・注目天体、ツアーの視線ガイド (空きを探して優先表示)
  ...
  const LBL_MAJOR = new Set(["sun", "mercury", "venus", "earth", "moon", "mars",
  ...
  function lblEnd() {
    ...
    const focusLabels = lblQ.filter(L => L.pri === LBL_SEL);
    octx.save();
    octx.shadowColor = "rgba(3,8,18,.9)";
    octx.shadowBlur = 3;
    for (const L of lblQ) {
      ...
      const hw = w * 0.5 + 8;
      const top = L.f.px * 0.85 + 2, bottom = L.f.px * 0.3 + 2;
      let x = L.x, y = L.y, placed = false;
      // 選択・注目名は画面内へ収め、衝突時には近い上段から空きを探す。
      // 通常名は動かさず間引く。大量の引き出し線で空を埋めないため。
      const important = L.pri === LBL_SEL;
      const attempts = important ? 9 : 1;
      for (let i = 0; i < attempts; i++) {
        x = important ? Math.max(hw + 4, Math.min(W - hw - 4, L.x)) : L.x;
        const row = Math.ceil(i / 2), dy = row * (top + bottom + 9);
        y = L.y + (i % 2 ? -dy : dy);
        if (important) y = Math.max(top + 4, Math.min(H - bottom - 4, y));
        const x0 = x - hw, x1 = x + hw, y0 = y - top, y1 = y + bottom;
        if (x0 < 0 || x1 > W || y0 < 0 || y1 > H) continue;
        let hit = false;
        for (const box of lblBox) {
          if (L.pri >= box[4] && x0 < box[2] && x1 > box[0] && y0 < box[3] && y1 > box[1]) { hit = true; break; }
        }
        if (hit) continue;
        // Reserve breathing room as well as the glyph bounds. Background names
        // yield first; important names can move to the next row.
        const gapX = important ? 12 : L.pri >= LBL_SAT ? 6 : 2;
        const gapY = important ? 5 : L.pri >= LBL_SAT ? 4 : 2;
        lblRect(x0-gapX, y0-gapY, x1+gapX, y1+gapY, 0);
        placed = true;
        break;
      }
      if (!placed) continue;
      if (important && Math.hypot(x - L.x, y - L.y) > 4) {
        octx.save();
        octx.strokeStyle = L.col; octx.globalAlpha = L.alpha * 0.35;
        octx.lineWidth = 0.75;
        octx.beginPath(); octx.moveTo(L.x, L.y + 3);
        octx.lineTo(x, y > L.y ? y - top : y + bottom); octx.stroke();
        octx.restore();
      }
      octx.font = L.f.s;
      octx.fillStyle = L.col;
      let emphasis = 1;
      if (!important && L.pri >= LBL_PROBE) {
        for (const focus of focusLabels) {
          const distance = Math.hypot(x-focus.x, y-focus.y);
          const u = Math.min(1, distance/140);
          emphasis = Math.min(emphasis, .55 + .45*u*u*(3-2*u));
        }
      }
  ...
  function nightAmbient(b, radiusPx, shine) {
    const v = b.comet
      ...
      : (() => {
          ...
          // 宇宙ビューでは暗部にごく弱い補助光を残して、背景との境界を読めるようにする。
          // 地上・月面からの観測と、地球照・彗星核の明るさは従来のまま。
          const floor = groundView ? 0.09 : 0.13;
          return 0.30 - (0.30 - floor) * (t * t * (3 - 2 * t));
        })();
