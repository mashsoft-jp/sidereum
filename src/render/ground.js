  ...
  function renderGround(nowSec) {
    ...
    if (bloomOn) {
      ...
      if (sv && sinAlt > 0) {
        ...
        // 決める。半径 0.9° は、これまで昼に裾が飽和させていた塊とほぼ同じ広さ
        // (半径 0.4° まで寄せたら「日没前から太陽が小さくなった」と言われた
        // 2026-09-08)。頂が平らで減光を緩く受けるので、沈んでも同じ大きさのまま
        // 白→黄→橙と色だけ変わる。月面は大気が無いので従来どおり
        // 日食では芯を欠けた側へ寄せて縮める。平らな頂は円盤より広いので、月面ほど
        // 緩く縮めると (1 − 食分×0.7) 三日月の上に白い円として残る — 地上では
        // (1 − 食分)² で強く縮め、食分 0.4 で半径 0.3° (三日月の幅) まで落とす
        const cw = atm ? 0.30 : 0.22;
        const csh = atm ? (1 - sunCov) * (1 - sunCov) : 1 - sunCov * 0.7;
