  ...
  const gM64F = new Float64Array(16);   // 上を地平ぎわの扁平で潰したもの (投影用)
  let groundVB = null, groundPtVB = null, horizonVB = null, horizonN = 0, groundDomeN = 0;
  // 風景表現 (稜線つきドーム + 空ドーム)。showTerrain が OFF なら遮蔽せず全天を表示する
  ...
  function refractUp(u) {
    if (!showTerrain || surfaceBody === "moon") { _rf[0] = 1; _rf[1] = u; return _rf; }   // 大気が無い
  ...
  // ---------- 地平ぎわの扁平 ----------
  // 大気差は高度が低いほど大きいので、円盤の下縁は上縁より強く持ち上げられ、
  // 縦だけが縮む。地平の太陽は 32′×32′ が 27′×32′ の楕円になる (縦に 17%) —
  // 夕日が横長のパンのように見えるのはこれ。
  //
  // 潰すのは「どこに見えるか」だけで、照らされ方は変えない。大気差は天体と目の
  // あいだで起きるので、天体表面の入射角・出射角には効かないため。そこで
  // uMVP だけを潰し、uModel (法線と照明に使う) は素のまま渡す。
  //
  ...
  function flatK(sinAlt) {
    ...
    const e = 1e-5;
    const k = 1 + (refractRad(h + e) - refractRad(h - e)) / (2 * e);
    return Math.max(0.5, Math.min(1, k));              // 地平線より下の発散よけ
  }
  // m の 3x3 を、視線に直交する「上」の向きにだけ k 倍する (中心は動かさない)。
  // 天体の姿勢とは無関係な、地平フレームでの潰し方であることに注意
  function flattenMat(m, dx, dy, dz, k, out) {
    for (let i = 0; i < 16; i++) out[i] = m[i];
    if (k >= 0.9999) return out;
    const dl = Math.hypot(dx, dy, dz) || 1;
    const ny = dy / dl;
    // 天頂 (0,1,0) から視線方向の成分を抜いたもの = 画面の「上」
    let ux = -dx / dl * ny, uy = 1 - ny * ny, uz = -dz / dl * ny;
    const ul = Math.hypot(ux, uy, uz);
    if (ul < 1e-4) return out;                         // ほぼ天頂。潰す向きが決まらない
    ux /= ul; uy /= ul; uz /= ul;
    const f = 1 - k;
    for (let c = 0; c < 3; c++) {
      const o = c * 4;
      const d = ux * out[o] + uy * out[o + 1] + uz * out[o + 2];
      out[o] -= f * ux * d; out[o + 1] -= f * uy * d; out[o + 2] -= f * uz * d;
    }
    return out;
  }
  ...
  function groundDsoAt(i, out) {
    ...
    if (showTerrain && up < 0.005) return false;
    ...
    if (showTerrain && surfaceBody !== "moon") { // 空と地形OFFでは大気減光も省く
      const eo = extIdx(up);
  ...
  function renderGround(nowSec) {
    ...
    // 太陽を中心に直径 25° ほどの白い塊になる。「空と地形」OFF の空は昼でも暗いままなので
    ...
    drawMilkyWay(gVP32, mwEqGround(), SKYR * 1.4,
                 ...
                 (isMoonSurf || !showTerrain) ? 0 : 1, (isMoonSurf || !showTerrain) ? null : EXT_K, _zodi);
    ...
    if ((!immersiveView || screensaverSky()) && showConst && CONST_SEG.length) {
      ...
      for (let i = 0; i + 5 < CONST_SEG.length; i += 6) {
        ...
        if (showTerrain && u0 < -0.03 && u1 < -0.03) continue;      // 両端とも地平線下ならスキップ
      ...
      // 黄道 (金色。ワールド単位 = ECL_WORLD/1895 を投影)
      {
        ...
        for (let k = 0; k < ECL_N; k++) {
          ...
          if (showTerrain && u0 < -0.03 && u1 < -0.03) continue;
          let rf = refractUp(u0); const a0 = rf[0] * SKYR, v0 = rf[1] * SKYR;
          rf = refractUp(u1);     const a1 = rf[0] * SKYR, v1 = rf[1] * SKYR;
    ...
    perfLap("星座線");
    // 恒星 (実カタログ。空と地形OFFでは地平線下も表示)。ワールド単位方向を観測者フレームへ投影
    if (N_CAT) {
      if (!starGVB) starGVB = gl.createBuffer();
      // 大気の無い月面では減光 (EXT_TBL) が効かないので、地上より一段
      // 大きく・明るく描く
      const extT = (isMoonSurf || !showTerrain) ? null : EXT_TBL;
      ...
      for (let i = 0; i < N_CAT; i++) {
        ...
        if (showTerrain && up < 0.02) continue;
        const east = wx*obsE[0]+wy*obsE[1]+wz*obsE[2];
        const north = wx*obsN[0]+wy*obsN[1]+wz*obsN[2];
        const rf = refractUp(up), hz = rf[0] * SKYR;
        const o = ns * 7;
    ...
    if (bigBodies.length) {
      ...
      for (const bb of bigBodies) {
        ...
        // 投影だけを潰す。uModel は素のままなので、法線も照明も変わらない
        const kF = flatK(bb.sa);
        const mvp = mMul(gVP, flattenMat(m, bb.px, bb.py, bb.pz, kF, gM64F), SCR.mvp);
        ...
        if (b.air) {
          ...
          airK = kF; airPx = bb.px; airPy = bb.py; airPz = bb.pz;
          airBody = b;
        }
      }
      // 大気は加算で重ねるので、本体をすべて描き終えてから最後に足す
      if (airBody) {
        const airMvp = mMul(gVP, flattenMat(SCR.air64, airPx, airPy, airPz, airK, gM64F), SCR.airMvp);
      ...
      if (satBB) {
        ...
        // 扁平は本体と同じ倍率で潰す。大きさによらず縦を一律に縮める効果なので、
        // 環だけ真円のまま残すと地平ぎわで本体と食い違う
        const kR = flatK(satBB.sa);
        gl.uniformMatrix4fv(ringP.u.uMVP, false,
                            mMul(gVP, flattenMat(m, satBB.px, satBB.py, satBB.pz, kR, gM64F), SCR.mvp));
        SCR.model.set(m);
    ...
    if (bloomOn) {
      ...
      if (sv && sinAlt > 0) {
        ...
        // 高度 5° で 1/3、2° で 1/6 に落ち、飽和している範囲 (= 見た目の太陽の
        // 大きさ) が沈むにつれて縮む (「太陽が小さくなっていく」2026-09-08)。
        // 0.3 乗で色の比を残しつつ落ち方を緩める (5° で 0.7、地平で 0.26)。
        // 板の大きさ・形・色は触らない — 芯を平らな頂にして円盤に寄せる案は、
        // 昼の太陽が白一色になって輝きが消えたので戻した
        const GP = 0.3;
        const fade = (ge ? Math.pow(ge[0], GP) : 1) * (1 - sunCov);
        const gm = ge ? Math.pow(ge[1] / Math.max(ge[0], 1e-6), GP) : 1;
        const bm = ge ? Math.pow(ge[2] / Math.max(ge[0], 1e-6), GP) : 1;
  ...
  function drawGroundOverlay() {
    ...
    if (showConst && starVis > 0.04) {
      ...
      for (const c of CONST_LABELS) {
        ...
        for (let i = 0; i + 2 < v.length; i += 3) {
          ...
          const up = wx*obsU[0]+wy*obsU[1]+wz*obsU[2];
          if (showTerrain && up < 0.03) continue;
    ...
    for (const v of groundVis) {
      ...
      if (hit && v.rpx < H * 0.3) {
        // 印は円盤に沿わせる。地平ぎわで天体だけ潰れていると、真円の印が
