  ...
  function renderGround(nowSec) {
    ...
    if (bloomOn) {
      ...
      if (sv && sinAlt > 0) {
        ...
        // 芯 (円盤の眩しさ) は減光をそのままは受けない。太陽面の輝度は空の 10万倍
        // あり、地平ぎわの 1/100 の減光でも画面では飽和したままなのが実際 — 白から
        // 黄・橙へ色だけ変わり、赤い円盤として見えるのは最後の 1° だけ。透過率の
        // 0.3 乗で色の比を残しつつ落ち方を緩める (5° で 0.7、地平で 0.26)。
        // 裾は目の中の散乱で光量に比例するので、こちらは透過率のまま
        const cfade = (ge ? Math.pow(ge[0], 0.3) : 1) * (1 - sunCov);
        const cgm = ge ? Math.pow(ge[1] / Math.max(ge[0], 1e-6), 0.3) : 1;
        const cbm = ge ? Math.pow(ge[2] / Math.max(ge[0], 1e-6), 0.3) : 1;
        ...
        // 地上 (大気あり) の裾は弱く、落ち方を急にする。昼の空は太陽のまわりが
        // ミー散乱ですでに 0.8 ほど明るいので、そこへ 0.9 を足すと直径 3° が白く
        // 飽和して「大きな太陽」になり、沈むにつれて減光で縮んで見える
        ...
        // 地上では芯を円盤の大きさへ寄せる (半径 0.40°。円盤は 0.27°)。平らな
        // 頂 (uFall 0.6) で円盤の上を一様に飽和させ、そのすぐ外で落とす —
        // 円盤の縁が見えたまま白→黄→橙と色が変わるので、沈む太陽が同じ
        // 大きさに見える。月面は大気が無いので従来どおり
        const cw = atm ? 0.13 : 0.22;
        gl.uniform1f(billP.u.uFall, atm ? 0.6 : 1.5);
        gl.uniform1f(billP.u.uSize, SKYR * GLARE_TAN * wide * cw * gz * (1 - sunCov * 0.7));
        const cf = atm ? cfade : fade, cg = atm ? cgm : gm, cb = atm ? cbm : bm;
        gl.uniform3f(billP.u.uCol1, 1.0 * cf, 0.80 * cf * cg, 0.45 * cf * cb);
        gl.uniform3f(billP.u.uCol2, 1.0 * cf, 1.00 * cf * cg, 1.00 * cf * cb);
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
        gl.disable(gl.BLEND);
        gl.disable(gl.DEPTH_TEST);
        gl.depthMask(true);
