  ...
  // 観察者側の斜め上から照らし、土星の縞と氷塊の凹凸を同じ光で見せる。
  const RING_CLOSE_LIGHT = [-.6, .45, .7];
  ...
  function stepRingExplore(dt) {
    ...
    if(ringExplore.phase && ringExplore.phase!=="inside" && ringExplore.phase!=="reveal")return;
  ...
  function renderRingExplore() {
    ...
    bodyRenderer.draw({body:sat,model,mvp:mMul(distantVP,model),sunPosition:RING_CLOSE_LIGHT.map(v=>v*1e13),radiusPx:H*.55});
    ...
    gl.uniform3f(ringCloseP.u.uLightDir,...RING_CLOSE_LIGHT);
