  ...
  function updateSpaceGuide(nowSec) {
    ...
    spaceGuide.body = b;
    const dt = spaceGuide.time === null ? 1 : Math.max(0, Math.min(.1, nowSec - spaceGuide.time));
    spaceGuide.time = nowSec;
    const blend = 1 - Math.exp(-dt / .45);
    spaceGuide.close += (close - spaceGuide.close) * blend;
    // 選択直後も線と名前を瞬時に消さず、同じ重みで徐々に整理する。
    for (const body of [...PLANETS, ...SATELLITES]) {
      const important = spaceRelated(body) || body.key === tourSpot || body.key === tourSight;
      const minor = body.H !== undefined && body.key !== "pluto";
      const target = important ? 1 : (selected && selected !== SUN ? .42 : minor ? .48 : 1);
      const previous = spaceGuide.weights.get(body.key) ?? 1;
  ...
  function spaceLabelAlpha(b) {
    return spaceRelated(b) || b.key === tourSpot || b.key === tourSight || b.mesh
      ? 1 : (1 - spaceGuide.close) * (spaceGuide.weights.get(b.key) ?? 1);
  }
  ...
  function orbitGuide(b) {
    ...
    gl.uniform1f(guideP.u.uFade, 0.85 * (spaceGuide.weights.get(b.key) ?? 1) * (1 - spaceGuide.close * (1 - keep)) * satelliteOrbitReadability(b));
