    ...
    void main() {
      ...
      if (uAtmos > 0.5) {
        ...
        float tau = path * 4.8 * exp(-max(b - 1.0, 0.0) / 0.0048);
      ...
      if (uComet > 0.5) {
        ...
        if (uType > 3.5 && uType < 4.5) {
          ...
          // 斜光では白さを少し早く落とし、雲の明暗が昼夜境界まで残るようにする。
          cloudLight = pow(max(mu0, 0.0), 0.7) * 0.64;
          alb *= 1.0 - cloud;
        }
      } else if (uType < 1.5) {
        // ---- 岩石 (テクスチャ無し小天体) ----
      ...
      if (isEarth > 0.5) {
        ...
        vec3 cloudTint = mix(vec3(1.0, 0.79, 0.61), vec3(1.0), smoothstep(0.0, 0.18, ndl));
        ...
        float limb = pow(1.0 - max(dot(N, V), 0.0), 3.8);
        ...
        c = c * exp(-vec3(0.32, 0.55, 0.95) * airPath) + air * airPath * 0.40;
