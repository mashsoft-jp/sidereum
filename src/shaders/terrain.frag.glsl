    ...
    float craterShape(float d) {
      ...
      return 0.30 * rim - 0.55 * bowl;
    }
    // 高さと傾斜を返す。中心探しは一度だけ行い、放射方向の傾斜から法線を作る。
    vec3 crater(vec4 v) {
      float rad = mix(0.13, 0.34, v.y);
    ...
    // 近いセルへ切り替えるとクレーターが途中で切れるため、各円形の起伏を重ねる。
    vec3 craterField(vec2 p) {
      ...
      for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
          ...
          vec2 delta = f - o - vec2(hash(cc), hash(cc + vec2(17.3, 31.7)));
          float dist = length(delta), id = hash(cc + vec2(5.2, 9.1));
          if (dist < mix(0.13, 0.34, id) * 1.38) result += crater(vec4(dist, id, delta));
        }
      }
      return result;
    }
    ...
    void main() {
      ...
      if (uMoon > 0.5) {
        ...
        vec3 k2 = craterField(g * 0.5) * (1.0 - smoothstep(0.3, 1.2, footprint * 0.5));
        vec2 slope = k1.yz * 0.12 * 0.45 + k2.yz * 0.5 * 0.10;
        vec3 normal = normalize(vec3(-slope.x, 1.0, -slope.y));
        float sh = 0.68 + 0.15 * base + 0.12 * grain;
        // 凹部をわずかに暗くし、縁の向きに応じた直射光で起伏を見せる。
        sh *= 1.0 + min(0.0, k1.x + k2.x * 0.4) * 0.3;
