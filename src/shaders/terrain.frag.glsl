    ...
    void main() {
      ...
      if (uMoon > 0.5) {
        ...
        // 観察用の補助光。太陽の無い側・影の中でも起伏を読める明るさを残す。
        // 日向では消えるので、直射光が十分ある場所の明るさは変えない。
        float dark = 1.0 - smoothstep(0.0, 0.25, lit * direct);
        float reliefFill = 0.55 + 0.45 * max(dot(normal, normalize(vec3(0.4, 1.0, 0.6))), 0.0);
        col *= 0.025 + lit * direct * 1.15 + 0.10 * dark * reliefFill;
      ...
      if (uMoon < 0.5) {
        ...
        vec3 haze = mix(vec3(0.010, 0.012, 0.017), vec3(0.075, 0.105, 0.14), uSkyF);
        ...
        float ridgeLit = smoothstep(-0.03, 0.06, uSun.y) * uOcc;
        vec3 ridge = vec3(0.10, 0.095, 0.087) * (0.025 + face * ridgeLit + 0.09 * (1.0 - ridgeLit));
