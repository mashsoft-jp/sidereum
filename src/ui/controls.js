  ...
  glc.addEventListener("pointermove", (e) => {
    if (!pointers.has(e.pointerId)) {
      glc.classList.toggle("hover", !!hitTest(e.clientX, e.clientY) || hitTestDso(e.clientX, e.clientY) >= 0 || !!hitTestNamedStar(e.clientX, e.clientY));
      return;
    }
  ...
  function endPointer(e) {
    ...
    if ((!tourActive || tourAllowsSelect()) &&
      ...
      // リストのボタンと同じ扱い
      const bodyHit = groundView ? hitTestGround(e.clientX, e.clientY) : hitTest(e.clientX, e.clientY);
      const starHit = !bodyHit ? hitTestNamedStar(e.clientX, e.clientY) : null;
      const skyHit = !tourActive && !bodyHit ? hitTestDso(e.clientX, e.clientY) : -1;
      if (starHit) { openNamedStar(starHit); }
      else if (skyHit >= 0) { openDsoPhoto(skyHit); }
      else if (groundView) {
        ...
        if (hit === selected && hit) toggleSelChrome(hit);
          ...
          // 何もない所を押したら選択を外し、追尾もやめる (宇宙ビューと同じ。
          // 地上だけ外れないままだった 2026-09-08)。向きはその場に残す
          select(null, false);
          gTrack = false;
        }
      } else {
