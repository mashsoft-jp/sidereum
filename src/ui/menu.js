  ...
  function drawSnapshotCaption(x, width, height, scale, when, site) {
    ...
    shade.addColorStop(0.55, "rgba(4,6,14,0.32)");
    shade.addColorStop(1, "rgba(4,6,14,0.68)");
    x.fillStyle = shade; x.fillRect(0, height - footer - pad, width, footer + pad);
    x.textBaseline = "bottom";
    x.shadowColor = "rgba(0,0,0,0.6)"; x.shadowBlur = 2;
    const logoY = compact ? bottom - metaRows * 18 - 12 : bottom - 18;
    x.font = "400 16px Megrim, system-ui, sans-serif";
    x.fillStyle = "rgba(242,178,62,0.82)";
    x.textAlign = compact ? "left" : "right";
    x.fillText("S I D E R E U M", compact ? pad : width - pad, logoY);
    x.font = "9px system-ui, -apple-system, sans-serif";
    x.fillStyle = "rgba(201,213,234,0.52)"; x.textAlign = "right";
    x.fillText("\u00a9 2026 Mashsoft Inc.", width - pad, compact ? logoY : bottom);
    x.textAlign = "left";
    const metaWidth = compact ? width - pad * 2 : width - pad * 2 - 180;
    // 観測地名が長い場合もロゴに重ならない。全文は共有リンクに保持される。
    const line = (text, y) => {
      let label = text;
      if (x.measureText(label).width > metaWidth) {
        while (label && x.measureText(label + "…").width > metaWidth) label = label.slice(0, -1);
        label += "…";
      }
      x.fillText(label, pad, y);
    };
    x.font = "11px system-ui, -apple-system, sans-serif";
    x.fillStyle = "rgba(201,213,234,0.78)";
    line(when, bottom - (site ? 18 : 0));
    if (site) {
      x.fillStyle = "rgba(201,213,234,0.60)";
      line(site, bottom);
    }
    x.restore();
  }
  ...
  function buildAbout() {
    ...
    aboutEl.innerHTML =
      ...
      `<p><a href="src/fonts/Megrim-OFL.txt" target="_blank" rel="noopener">Megrim — SIL Open Font License 1.1</a><br>` +
      `<a href="THIRD_PARTY_NOTICES.md" target="_blank" rel="noopener">${lang === "ja" ? "第三者素材の権利表記・解説の参考資料" : "Third-party notices and description references"}</a></p><p>${c.disc}</p>`;
  }
