  ...
  function saverPhotoLayout(width, height, imageWidth, imageHeight, captionHeight, topInset, bottomInset) {
    const gap = 12, side = 12;
    const availableHeight = Math.max(1, height-topInset-bottomInset-captionHeight-gap);
    const scale = Math.min(Math.max(1,width-side*2)/imageWidth, availableHeight/imageHeight);
  ...
  function layoutSaverPhoto() {
    ...
    const layout = saverPhotoLayout(W,H,saverPhotoImage.naturalWidth,saverPhotoImage.naturalHeight,
      saverBar.getBoundingClientRect().height,parseFloat(padding.paddingTop)||16,parseFloat(padding.paddingBottom)||16);
  ...
  function moveSaverCamera(dt) {
    ...
    if (s.kind === "dsoPhoto" && s.photoReady) {
      ...
      saverPhotoImage.style.transform = "translateX(-50%) scale(" + (.96 + s.photoMotion * .04/34) + ")";
    }
