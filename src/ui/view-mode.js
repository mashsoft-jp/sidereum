  ...
  function aimGroundAt(body, instant) {
    ...
    // 即時に向けたときは、追尾の基準もこの瞬間の値へ入れ替える。古い基準を
    // 残すと、次のフレームで「前フレームからの天体の移動分」として日時ジャンプ
    // ぶんがもう一度足され、ずれた画から数秒かけて戻ってくる (ツアーで日を
    // 跨ぐ回に出ていた)。かといって空にすると、次のフレームは移動分の即時反映を
    // 飛ばして残差の緩和だけになるので、早回しでは1フレームぶん (読み込み中の
    // 長いフレームなら画面の1割) ずれた位置から寄っていく動きが見えてしまう
    if (instant) {
      gAz = gAzTgt; gAlt = gAltTgt;
      gTrkKey = body.key; gTrkAz = c.az * DEG; gTrkAlt = c.alt * DEG;
    }
    return true;
  }
